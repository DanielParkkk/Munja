...
YOLO_ROOT = PROJECT_ROOT / "vendor" / "yolov5"
...
from . import classifier, iou_cal
...
def load_models(device_name=""):
    weights = MODELS_DIR / "weights_detector.pt"
    imgsz = (1280, 1280)
    ...
    t_data = pd.read_csv(DATA_DIR / "class.csv", header=None)
    t_data = t_data.iloc[:, [0, 1]]

    with open(DATA_DIR / "class.csv", "r", encoding="utf-8") as handle:
        class_num = int(handle.readlines()[-1].split(",")[0]) + 1
    ...
    classifier_model.load_state_dict(torch.load(MODELS_DIR / "weights_classifier.pth", map_location=cls_device))
...
def extract_ancient_text_with_models(image_path, models):
    detector, classifier_model, t_data, transform, device, _, imgsz, stride, pt = models
    ...
    for _, im, im0s, _, _, _, _, _ in dataset:
        ...
        for det in pred:
            ...
            pred_class_list = classifier.get_predictions(classifier_model, device, ocrloader, [])
...
def parse_opt():
    ...
    parser.add_argument("--image", type=str, required=True, help="OCR를 실행할 이미지 파일 경로")
